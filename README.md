...
1. Install Python
The scanner requires Python 3.8 or higher.
...
Note: Ensure you check "Add Python to PATH" during setup.
...
Install a WebDAV Server app on your mobile device or PC.
...
1.Si encuentras algún error durante la ejecución, sigue estos pasos:

2.Revisa la documentación – consulta rápidamente este README para verificar la configuración.

3.Verifica la conexión WebDAV – asegúrate de que el servidor esté activo y que la IP, puerto, usuario y contraseña sean correctos.Inspecciona los logs – observa la salida en consola y el archivo scanner.log; los mensajes de error suelen indicar la causa.

5.Consulta a una IA o busca en línea – si el error persiste, pega el mensaje de error en un buscador o pide ayuda a una IA.

If you encounter any errors during execution, follow these steps:

1.Check the documentation – quickly review this README for setup instructions.

2.Verify the WebDAV connection – ensure your server is running, and the IP, port, username, and password are correct.

3.Inspect the logs – look at the console output and the scanner.log file; error messages usually point to the issue.

4.Consult an AI or search online – if the error persists, paste the error message into a search engine or ask an AI for help.
